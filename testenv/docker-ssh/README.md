...
| --- | --- | --- |
| Linux (`ubuntu-latest`) | `ubuntu:24.04` ×2 | **CI** — main / `cut` (`E2E`) and weekly |
...
`./scripts/up.sh --e2e` on `ubuntu-latest` for **main** (E2E-relevant paths),
`cut` PRs, and manual dispatch (`ubuntu-latest → ubuntu-24.04`). Ordinary PRs
skip that job's Docker steps.
...
same suite as `cut` / **main** Linux E2E, on the timer,
not a PR check. After a `cut` merge,
