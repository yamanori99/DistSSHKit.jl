...
julia --project=. -m DistSSHKit drive parent:2 demos/with_kit/square_echo.jl --n 4
...
Same tokens as the CLI: `pipeline!(driver, "parent:2"; args=[…])`. A commented remote
...
(comma-separated `host` or `child:NAME:N`; used by `go` / `drive` / `pipeline!`),
