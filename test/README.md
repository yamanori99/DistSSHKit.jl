...
| **ssh-e2e** (`E2E / Linux controller`) | Real SSH + rsync against Docker workers (`DISTSSHKIT_SSH_E2E=1`). CI: Linux controller on every PR | ~10–20 min |
| **ssh-e2e macOS** (`E2E / macOS controller`) | Same suite from **macOS Intel** + Colima. **main / dispatch** only. Pulls the worker image the Linux job pushed | ~25–50 min |
| **ssh-e2e WSL** (`E2E / WSL2 controller`) | Same suite from **WSL2 Ubuntu** to Linux Docker workers. **main / dispatch** only. Not native Windows | ~20–45 min |
...
Remote SSH on PRs is **E2E / Linux controller**. Merge-only: **E2E / macOS controller** and **E2E / WSL2 controller**. Doctests run in **Docs / Documenter (Julia 1.12)**, not `Pkg.test()`. **Test / Pkg.test** on 1.10/1.11 uses `DistSSHKit.main` for CLI children (`-m` is 1.12+).
