...
| **ssh-e2e macOS** (`E2E / macOS → Linux (skipped on PR)`) | Same suite from **macOS Intel** + Colima. **main / dispatch** only. Pulls the worker image the Linux job pushed | ~25–50 min |
| **ssh-e2e WSL** (`E2E / WSL2 → Linux (skipped on PR)`) | Same suite from **WSL2 Ubuntu** to Linux Docker workers. **main / dispatch** only. Not native Windows | ~20–45 min |
...
Remote SSH on PRs is **E2E / Linux → Linux**. Merge-only (grey skipped on PRs): **E2E / macOS → Linux (skipped on PR)** and **E2E / WSL2 → Linux (skipped on PR)**. Doctests run in **Docs / Documenter (Julia 1.12)**, not `Pkg.test()`. **Test / Pkg.test** on 1.10/1.11 uses `DistSSHKit.main` for CLI children (`-m` is 1.12+).
