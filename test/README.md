...
| **ssh-e2e macOS** (`E2E / macOS → Linux (main / dispatch)`) | Same suite from **macOS Intel** + Colima. Pulls the worker image the Linux job pushed | ~25–50 min |
| **ssh-e2e WSL** (`E2E / WSL2 → Linux (main / dispatch)`) | Same suite from **WSL2 Ubuntu** to Linux Docker workers. Not native Windows | ~20–45 min |
...
Remote SSH on PRs is **E2E / Linux → Linux**. After merge (or dispatch): **E2E / macOS → Linux (main / dispatch)** and **E2E / WSL2 → Linux (main / dispatch)**. Doctests run in **Docs / Documenter (Julia 1.12)**, not `Pkg.test()`. **Test / Pkg.test** on 1.10/1.11 uses `DistSSHKit.main` for CLI children (`-m` is 1.12+).
