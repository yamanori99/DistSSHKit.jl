...
| **ssh-e2e macOS** (`E2E / macOS → Linux (schedule / dispatch)`) | Same suite from **macOS Intel** + Colima. Daily 06:00 JST or dispatch. Pulls the worker image the Linux job pushed | ~25–50 min |
| **ssh-e2e WSL** (`E2E / WSL2 → Linux (schedule / dispatch)`) | Same suite from **WSL2 Ubuntu** to Linux Docker workers. Daily / dispatch. Not native Windows | ~20–45 min |
...
Remote SSH on PRs and main is **E2E / Linux → Linux**. Daily / dispatch: **E2E / macOS → Linux (schedule / dispatch)** and **E2E / WSL2 → Linux (schedule / dispatch)**. Doctests run in **Docs / Documenter (Julia 1.12)**, not `Pkg.test()`. **Test / Pkg.test** on 1.10/1.11 uses `DistSSHKit.main` for CLI children (`-m` is 1.12+).
